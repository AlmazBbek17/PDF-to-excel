...
def extract():
    ...
    prompt = """You are a data extraction engine. Your ONLY job is to convert this PDF page image into a JSON table.

You MUST always return a JSON array with at least one table. NEVER return []. This is mandatory.

Rules:
- ANY content on the page can be a table: financial data, lists, key-value pairs, plain text, anything
- If there are clear columns and rows — extract them exactly
- If there is no obvious table — put all text into a single-column table with header "Content"
- Financial reports: each line (label + numbers) is one row, column headers are the year/period labels
- Preserve all numbers and text exactly as shown
- Section headers (e.g. "Revenues:") → include as a row with empty other cells
- Empty cells → use ""
...
[
  {
    ...
    if not tables:
        tables = [{"title": f"Page {page_number}", "headers": ["Content"], "rows": [["No structured table data found on this page."]]}]
