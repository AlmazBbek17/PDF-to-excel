...
def extract():
    ...
    print(f"[DEBUG] Image b64 length: {len(image_b64)}, page: {page_number}", flush=True)
...
- Do NOT include any explanation, only the JSON array"""
    ...
    print(f"[DEBUG] Claude raw response: {response_text[:500]}", flush=True)
    tables = parse_tables_from_response(response_text)
    print(f"[DEBUG] Parsed tables count: {len(tables)}", flush=True)

    if not tables:
        print(f"[DEBUG] Fallback triggered — Claude returned no tables", flush=True)
